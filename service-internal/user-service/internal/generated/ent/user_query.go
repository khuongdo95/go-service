...
type UserQuery struct {
	...
	withIdentity    *IdentityQuery
	withIPWhiteList *UserIpWhiteListQuery
...
func (uq *UserQuery) QueryIdentity() *IdentityQuery {
...
// QueryIPWhiteList chains the current query on the "ip_white_list" edge.
func (uq *UserQuery) QueryIPWhiteList() *UserIpWhiteListQuery {
	query := (&UserIpWhiteListClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withIdentity:    uq.withIdentity.Clone(),
		withIPWhiteList: uq.withIPWhiteList.Clone(),
...
func (uq *UserQuery) WithIdentity(opts ...func(*IdentityQuery)) *UserQuery {
	query := (&IdentityClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withIdentity = query
	return uq
}

// WithIPWhiteList tells the query-builder to eager-load the nodes that are connected to
// the "ip_white_list" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithIPWhiteList(opts ...func(*UserIpWhiteListQuery)) *UserQuery {
	query := (&UserIpWhiteListClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withIPWhiteList = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withIdentity; query != nil {
		if err := uq.loadIdentity(ctx, query, nodes,
			func(n *User) { n.Edges.Identity = []*Identity{} },
			func(n *User, e *Identity) { n.Edges.Identity = append(n.Edges.Identity, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withIPWhiteList; query != nil {
		if err := uq.loadIPWhiteList(ctx, query, nodes,
			func(n *User) { n.Edges.IPWhiteList = []*UserIpWhiteList{} },
			func(n *User, e *UserIpWhiteList) { n.Edges.IPWhiteList = append(n.Edges.IPWhiteList, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadIdentity(ctx context.Context, query *IdentityQuery, nodes []*User, init func(*User), assign func(*User, *Identity)) error {
	...
	query.Where(predicate.Identity(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.IdentityColumn), fks...))
	}))
...
func (uq *UserQuery) loadIPWhiteList(ctx context.Context, query *UserIpWhiteListQuery, nodes []*User, init func(*User), assign func(*User, *UserIpWhiteList)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(useripwhitelist.FieldUserID)
	}
	query.Where(predicate.UserIpWhiteList(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.IPWhiteListColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
